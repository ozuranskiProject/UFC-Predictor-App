...
export default function Counter( {internalCount = 0, onValueChange, testID}) {    //This is the function "Counter"    //startValue is a "prop" (can change variable from App.js)
  ...
  const handleIncrement = () => {    //This function will track the count in order to relay it in a prop to the parent or App.js
  ...
  return ( //*-_-_-in react, return is mostly UI outputs for components to render. here its all in JSX, a layout syntax-_-_-*  
    ...
    <View style={styles.container}>                                                 
      <Text style={styles.counterText} testID={`${testID}-label`}>Count: {internalCount}</Text>                 
